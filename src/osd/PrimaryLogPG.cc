...
void PrimaryLogPG::log_op_stats(OpContext *ctx)
{
  ...
  if (op->may_read() && op->may_write()) {
    ...
    utime_t shard_lat = ctx->op->pg_lock_before - ctx->op->get_req()->get_recv_stamp();
    ...
    // if (ctx->op->sub_op_commit_rec >= ctx->op->op_commit) {
    if (ctx->op->repop_commit_rec.front() >= ctx->op->op_commit) {
      utime_t waitsubop_lat = ctx->op->repop_commit_rec.front() - ctx->op->op_commit;
      osd->logger->tinc(l_osd_op_w_waitsubop_lat, waitsubop_lat);
    }
    // } else {
    //   ctx->op->sub_op_commit_rec = ctx->op->op_commit;
    // }

    // utime_t lastsubop_lat = now - ctx->op->sub_op_commit_rec;
    if (!ctx->op->repop_commit_rec.empty()) {
      utime_t lastsubop_lat = ctx->op->repop_commit_rec.back() - ctx->op->repop_commit_rec.front();
